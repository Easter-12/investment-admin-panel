...
function WithdrawalsContent() {
  ...
  const handleUpdateRequest = async (requestId: number, newStatus: 'approved' | 'rejected') => {
    ...
    if (newStatus === 'approved') {
      try {
        ...
        // 1. Read the keys using the NEXT_PUBLIC_ prefix, which our new config file provides.
        const serviceID = process.env.NEXT_PUBLIC_EMAILJS_SERVICE_ID;
        const templateID = process.env.NEXT_PUBLIC_EMAILJS_TEMPLATE_ID;
        const publicKey = process.env.NEXT_PUBLIC_EMAILJS_PUBLIC_KEY;

        if (!serviceID || !templateID || !publicKey) {
          throw new Error("EmailJS keys were not loaded correctly from next.config.js. Check the file and your secrets.");
        }

        // 2. Approve the withdrawal in the database
        ...
        const templateParams = {
          ...
          to_email: request.profiles.email,
          from_name: "QuantumLeap Support",
        };

        // 3. Send the email using the verified keys
        ...
        setMessage('Error: ' + (err.text || err.message));
      }
  ...
  return (
    <div style={{ padding: '4rem', color: 'white' }}>
      <h1 style={{ fontSize: '2.5rem', fontWeight: 'bold', color: 'white', borderBottom: '1px solid #374151', paddingBottom: '1rem' }}>Manage Withdrawals</h1>
      {message && <p style={{ marginTop: '1rem', color: message.startsWith('Error') ? '#f87171' : '#34d399' }}>{message}</p>}
      <div style={{ marginTop: '2rem', overflowX: 'auto' }}> {loading ? <p>Loading requests...</p> : ( <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '900px' }}> <thead> <tr style={{ borderBottom: '1px solid #374151' }}> <th style={{ padding: '1rem', textAlign: 'left', color: '#9ca3af' }}>User</th> <th style={{ padding: '1rem', textAlign: 'left', color: '#9ca3af' }}>Amount</th> <th style={{ padding: '1rem', textAlign: 'left', color: '#9ca3af' }}>Wallet Address</th> <th style={{ padding: '1rem', textAlign: 'left', color: '#9ca3af' }}>Date</th> <th style={{ padding: '1rem', textAlign: 'left', color: '#9ca3af' }}>Status</th> <th style={{ padding: '1rem', textAlign: 'left', color: '#9ca3af' }}>Actions</th> </tr> </thead> <tbody> {requests.map((req) => ( <tr key={req.id} style={{ borderBottom: '1px solid #374151' }}> <td style={{ padding: '1rem', fontWeight: 'bold' }}>{req.profiles.username}</td> <td style={{ padding: '1rem' }}>${req.amount.toFixed(2)}</td> <td style={{ padding: '1rem', fontFamily: 'monospace', fontSize: '0.8rem' }}>{req.wallet_address}</td> <td style={{ padding: '1rem' }}>{new Date(req.created_at).toLocaleString()}</td> <td style={{ padding: '1rem' }}><span style={{padding: '0.25rem 0.75rem', borderRadius: '99px', fontSize: '0.8rem', fontWeight: 'bold', color: 'white', backgroundColor: req.status === 'pending' ? '#ca8a04' : req.status === 'approved' ? '#16a34a' : '#dc2626'}}>{req.status}</span></td> <td style={{ padding: '1rem' }}> {req.status === 'pending' && ( <div style={{display: 'flex', gap: '0.5rem'}}> <button onClick={() => handleUpdateRequest(req.id, 'approved')} style={{backgroundColor: '#16a3aa', color: 'white', border: 'none', padding: '0.5rem 1rem', borderRadius: '6px', cursor: 'pointer'}}>Approve</button> <button onClick={() => handleUpdateRequest(req.id, 'rejected')} style={{backgroundColor: '#dc2626', color: 'white', border: 'none', padding: '0.5rem 1rem', borderRadius: '6px', cursor: 'pointer'}}>Reject</button> </div> )} </td> </tr> ))} </tbody> </table> )} </div>
    </div>
  );
}
