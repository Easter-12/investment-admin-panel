...
// Define a type for our plan data
type Plan = {
  id: number;
  name: string;
  min_deposit: number;
...
function PlansContent() {
  ...
  // --- 2. Fetch plans when the page loads ---
  useEffect(() => {
    fetchPlans();
  }, []);

  // --- 3. Function to Handle Creating a New Plan ---
  const handleCreatePlan = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Clear the form and refresh the list of plans
      setName(''); setMinDeposit(''); setMaxDeposit(''); setDuration(''); setRoi('');
      fetchPlans();
    } catch (err: any) {
      setMessage('Error creating plan: ' + err.message);
    }
  };

  // --- 4. Function to Handle Deleting a Plan ---
  const handleDeletePlan = async (planId: number) => {
    if (window.confirm('Are you sure you want to delete this plan?')) {
      try {
        ...
        setMessage('Error deleting plan: ' + err.message);
      }
    }
  };
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '800px' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #374151' }}>
                <th style={{ padding: '1rem', textAlign: 'left', color: '#9ca3af' }}>Name</th>
                <th style={{ padding: '1rem', textAlign: 'left', color: '#9ca3af' }}>Min-Max Deposit</th>
                <th style={{ padding: '1rem', textAlign: 'left', color: '#9ca3af' }}>Duration</th>
                <th style={{ padding: '1rem', textAlign: 'left', color: '#9ca3af' }}>ROI</th>
                <th style={{ padding: '1rem', textAlign: 'left', color: '#9ca3af' }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {plans.map((plan) => (
                <tr key={plan.id} style={{ borderBottom: '1px solid #374151' }}>
                  <td style={{ padding: '1rem' }}>{plan.name}</td>
                  <td style={{ padding: '1rem' }}>${plan.min_deposit} - ${plan.max_deposit}</td>
                  <td style={{ padding: '1rem' }}>{plan.duration_days} days</td>
                  <td style={{ padding: '1rem' }}>{plan.roi_percentage}%</td>
                  <td style={{ padding: '1rem' }}>
                    <button onClick={() => handleDeletePlan(plan.id)} style={{backgroundColor: '#ef4444', color: 'white', border: 'none', padding: '0.5rem 1rem', borderRadius: '6px', cursor: 'pointer'}}>Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
