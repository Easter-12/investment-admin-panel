...
function WalletsContent() {
  ...
  const handleCreateWallet = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCryptoName(''); setCryptoSymbol(''); setWalletAddress('');
      fetchWallets();
    } catch (err: any) {
      setMessage('Error creating wallet: ' + err.message);
    }
  };
  ...
  const handleDeleteWallet = async (walletId: number) => {
    if (window.confirm('Are you sure you want to delete this wallet?')) {
      try {
        ...
        setMessage('Error deleting wallet: ' + err.message);
      }
    }
  };
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '800px' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #374151' }}>
                ...
                <th style={{ padding: '1rem', textAlign: 'left', color: '#9ca3af' }}>Symbol</th>
                <th style={{ padding: '1rem', textAlign: 'left', color: '#9ca3af' }}>Address</th>
                <th style={{ padding: '1rem', textAlign: 'left', color: '#9ca3af' }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {wallets.map((wallet) => (
                <tr key={wallet.id} style={{ borderBottom: '1px solid #374151' }}>
                  <td style={{ padding: '1rem' }}>{wallet.crypto_name}</td>
                  <td style={{ padding: '1rem' }}>{wallet.crypto_symbol}</td>
                  <td style={{ padding: '1rem', fontFamily: 'monospace' }}>{wallet.wallet_address}</td>
                  <td style={{ padding: '1rem' }}>
                    <button onClick={() => handleDeleteWallet(wallet.id)} style={{backgroundColor: '#ef4444', color: 'white', border: 'none', padding: '0.5rem 1rem', borderRadius: '6px', cursor: 'pointer'}}>Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
